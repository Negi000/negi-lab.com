/**
 * 高品質BGM生成UI（改善版）- Realistic Tone Engine用
 */
class RealisticToneUI {
  ...
  displayInstrumentStatus() {
    ...
    instrumentsList.forEach(instrument => {
      const statusClass = instrument.status === 'loaded' ? 'loaded' : 
                         instrument.status === 'fallback' ? 'fallback' : 'loading';
      const statusIcon = instrument.status === 'loaded' ? '✓' : 
                        instrument.status === 'fallback' ? '⚠' : '⏳';
      
      html += `
        <div class="instrument-item ${statusClass}">
          <span class="status-icon">${statusIcon}</span>
          <span class="instrument-name">${instrument.name}</span>
          <span class="status-text">${this.getTranslation(instrument.status, instrument.status)}</span>
        </div>
      `;
    });
  ...
  getSettings() {
    ...
    if (this.elements.instrumentCheckboxes) {
      settings.instruments = Array.from(this.elements.instrumentCheckboxes)
